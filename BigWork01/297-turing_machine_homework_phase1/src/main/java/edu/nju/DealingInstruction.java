...
public class DealingInstruction {
    ...
    public Set<Character> InstructionAnalyzeS(String str){
        ...
        if(!(str.contains("{") && str.contains("}"))){
            ...
            if(!(s[0].equals("#S = "))){
                ...
                String[] ss = s[1].split("}")[0].split(",");
                for(int j=0;j<ss.length;j++){
                    if(ss[j].charAt(0) == ' '||ss[j].charAt(0) == ','||ss[j].charAt(0) == ';'||ss[j].charAt(0) == '{'||ss[j].charAt(0) == '}'||ss[j].charAt(0) == '*'||ss[j].charAt(0) == '_') {
                        System.err.printf("Error: %d\n",i);
                    }
                    else{
    ...
    public TransitionFunction InstructionAnalyzeD(String str){
        int i = 0;
        String[] s = str.split(" ");
        if(s.length!=6){
            for(i=0;i<ins.length;i++){
        ...
        TransitionFunction TF = new TransitionFunction(str.substring(3));
        return TF;
    }
}
